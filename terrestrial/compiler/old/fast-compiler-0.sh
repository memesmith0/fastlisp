...
prepare_text(){ busybox awk '{gsub(/001010000111010001100101011110000111010000000000/, "f"); print }' |
		   busybox awk '{gsub(/0000000000101001/, "f"); print }' ; } ;
...
cat "$1" |
    ...
    skip_empty_lines |
    break_file_to_characters |
    turn_characters_to_ints |
    turn_ints_to_binary |
    prepend_0 | busybox awk '{printf "%s", $0}' |
    ...
    ##one of these functions seems to be changing the output of my strings
    #these dont change line numbers
...
############################################################
    ...
    remove_text |
    skip_empty_lines |
    busybox awk '{if($0 == "lambda"){print "00"}else{print $0}}'|
