...
#busybox awk '{if(d){d+=(($0 == "(comment") || ($0 == "(") ? 1 : ($0 == ")") ? -1 : 0)}else{if($0 == "(comment"){d=1}else{print $0}}}' ;}

#busybox awk '{if(d){d+=(($0 == "(comment") || ($0 == "(") ? 1 : ($0 == ")") ? -1 : 0)}else{print $0=="(comment"?(d++):$0}}' ;}

#busybox awk '{print d ? (d+=($0 == "(comment" || $0 == "(" ? 1 : $0 == ")" ? -1 : 0)): $0=="(comment"?d++:$0}' ;}

#busybox awk '{if((d ? (d+=($0 == "(comment" || $0 == "(" ? 1 : $0 == ")" ? -1 : 0)): $0=="(comment"?d++:$0) !~ /^[+-]?[0-9]+$/){print}}' ;}

#busybox awk '{if((d ? (d+=($0 == "(comment" || $0 == "(" ? 1 : $0 == ")" ? -1 : 0)): $0=="(comment"?d++:$0) !~ /[0-9]/ ){print}}' ;}

#busybox awk '(d ? (d+=($0 == "(comment" || $0 == "(" ? 1 : $0 == ")" ? -1 : 0)): $0=="(comment"?d++:$0) !~ /[0-9]/' ;}

#busybox awk '(d ? (d+=($0 == "(comment" || $0 == "(" ? 1 : $0 == ")" ? -1 : 0)): $0=="(comment"?d++:$0) !~ /[0-9]/ && $0 != ""' ;}


#busybox awk '{c=$0;a="(comment"==c;if((d?(d+=(a||c=="("?1:c==")"?-1:0)):c=="(comment"?d++:c)!~/[0-9]/){print}}';}

#busybox awk '{c=$0;a="(comment"==c}(d?(d+=(a||c=="("?1:c==")"?-1:0)):c=="(comment"?d++:c)!~/[0-9]/';}

#busybox awk '{c=$0;a="(comment"==c}(d?(d+=(a||c=="("?1:c==")"?-1:0)):a?d++:c)!~/[0-9]/&&c!=""';}

#busybox awk '{a="(comment"==$0}(d?(d+=(a||$0=="("?1:$0==")"?-1:0)):a?d++:$0)!~/[0-9]/&&$0!=""';}


#busybox awk '{a="(comment"==$0}
#((d?   #if d
#(d+=  #add to d
#(  a||  ##if a is "(" 1 else if a is ")" -1 else 0
#
#
#        $0=="(" ?1: $0==")"  ?  -1  : 0   ))          : #else if not d
#a? #if $0 == "(comment"
#d++: #add 1 to d
##else print $0 but filter 0-9 and whitespace
#$0))                                                         !~/[0-9]/&&$0!=""';}
...
#busybox awk '{d+=((((d &&($0=="(")) || $0="(comment") ? 1:$0 == ")")?-1 : 0)
...
#busybox awk '{ print 0, $0 }' |
    
#busybox awk '{if(d)(d+=((((($0=="(")) || $0="(comment") ? 1:$0 == ")")?-1 : 0)) print}'  #|
    
#busybox awk '$1' |
#busybox awk '{$2}'    

#busybox awk '{d+=((((d &&($0=="(")) || $0="(comment") ? 1:$0 == ")")?-1 : 0) if(!d){print $0}}' #!~/[0-9]/&&$0!=""#


#awk 'BEGIN{d=0}{d+=(($0=="(comment")&&(d==0)) ? 1 :0;d+=(($0=="(" || $0=="(comment") && d>0) ? 1 : ($0 == ")" && d>0) ? -1 : 0;if(d==0){print $0}}'
...
for(i=1; i < (length($0) + 1); i++){
	 ...
	 if(c=="f"){
		...
		for(; c != "f" ;){
		       	  ...
		       	  printf " ( ( lambda x ( lambda  y ( lambda z ( z x y ) ) ) ) ( lambda x ( lambda y %s ) ) ", ((c == 1)?"y":"x");
			  j++
			  i++
			  c=substr($0, i, 1)
			  }
...
remove_text2(){ busybox awk '{if($1 == "text:"){print $2}else{print}}';}
...
phase_1(){     convert_newlines_to_spaces |
    convert_spaces_to_newlines |
   separate_left_paren |
...
phase_5(){
    ...
    prepend_text_to_nonbinary_lines  |
    add_newlines_between_characters_in_text_nodes |
    prepend_text_to_nonbinary_lines  |
    turn_text_nodes_to_ints |
    turn_text_nodes_to_binary |
...
compile_fastlisp(){
    cat "$1" | phase_1 | phase_2 | phase_3 | phase_4 | phase_5 | phase_6 | phase_7 | phase_8 | phase_9 | phase_10
}
