...
if($0=="lambda"){
       	    ...
       	    depths[depth,depths[depth,"counter"]++]="lambda\n" $0
	    }
	    else if($0 == "("){
	    	 depth++
		 }
		      ...
		      a="(\n"
		      b=")\n"
		      for(;depths[depth,"counter"];){
			c=c "01\n";
			d=d depths[depth,depths[depth, "counter"]--]
			delete depths[depth,depths[depth,"counter"]+1]
			}
			depth--;		
			depths[depth,depths[depth,"counter"]++]=a b c d;
			}
			else{
				depths[depth,depths[depth,"counter"]++]=$0}
...
#						print depths[0,0]
						      ...
						      }' # | #add 01 to horizontal application
#   awk '{
